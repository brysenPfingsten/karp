;;
;; content of this file is modified from the unstable collection
;; https://github.com/racket/unstable/blob/master/unstable-lib/error.rkt
;;
...
(provide/contract
 ...
 [compose-error-message
  (->* [(or/c symbol? #f) (or/c symbol? #f) string?]
       [#:continued (or/c string? (listof string))]
       #:rest details-list/c
       string?)]
...
(struct exn:fail/srcloc exn:fail
  (a-srcloc)
  #:property prop:exn:srclocs
  (λ (x)
    (list (exn:fail/srcloc-a-srcloc x))))


(define (error* where who message
                #:continued [continued-message null]
                . field+detail-list)
  (raise
   (exn:fail/srcloc
    (compose* where who message
              continued-message
...
(define (compose-error-message where who message
                               #:continued [continued-message null]
                               . field+detail-list)
  (define details
    (field+detail-list->table 'compose-error-message field+detail-list null))
  (compose* where who message continued-message details))
...
(define (compose* where who message continued-message details)
  (let* ([parts (apply append
         ...
         [parts (if who
                    ...
                    parts)]
         [parts (if where
                    (list* (srcloc->string where) ": " parts)
                    parts)])
