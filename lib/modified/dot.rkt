...
(require racket/class
         ...
         racket/system
         #;racket/pretty
         (only-in mrlib/graph graph-snip<%>))
...
(define (dot-positioning pb [option dot-label] [overlap? #f] [directed? #t])
  (define dot-path (find-dot (regexp-match #rx"neato" option)))
  (when dot-path
    (define info (snip-info pb))
    (define-values (positions max-y) (run-dot dot-path info option overlap? directed?))
...
(define (run-dot dot-path ht option overlap? [directed? #t])
  ...
  (let-values ([(in1 out1) (make-pipe)]
    ...
    (thread
     (λ ()
       (parameterize ([current-output-port out1])
         (graph->dot info option overlap? directed?))
    ...
    ; for debugging
    #;(pretty-print (graph->dot info option overlap? directed?))
    
    (parse-plain in2)))

;; graph->dot (listof (list snip number (listof number)) -> void
;; prints out the dot input file based on `g'
(define (graph->dot g option overlap? [directed? #t])
  (printf "~a {\n" (if directed? "digraph" "graph"))
  (cond
    [(equal? option dot-label)
     (when directed? (printf "  rankdir=\"~a\"\n" (if overlap? "LR" "TB")))]
  ...
  (for-each
   (λ (line)
     (let* ([snip (list-ref line 1)]
       ...
       (for-each
        (λ (child)
          (printf "  ~a ~a ~a\n" (num->id id) (if directed? "->" "--") (num->id child)))
        children)))
   g)
  (printf "}\n"))
