...
(define (find-tmp-dir)
  ...
  (cond
    [(has-read-write-access? tmp-dir) tmp-dir]
    [(and (not (equal? (current-directory) tmp-dir))
          (has-read-write-access? (current-directory)))
     (current-directory)]
    [else (error (format "Unable to write or read to directory ~s" (path->string tmp-dir)))]))
