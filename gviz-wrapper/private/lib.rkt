...
;; Converts a racket graph to an image
;; If clean is false then the dot and png files are not deleted
(define (racket-graph->bitmap graph #:directory [save-dir SAVE-DIR] #:filename[filename "__tmp__"] #:clean[delete-files #t])
  (define-values (img path)
    ((compose1 png->bitmap dot->png racket-graph->dot) graph save-dir filename))
  (when delete-files
